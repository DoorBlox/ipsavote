...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ voters, setVoters, onOpenQRSheet, onClearAll }) => {
  ...
  const handleCSVUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (nameIdx === -1 || roleIdx === -1) throw new Error("Missing 'Name' or 'Role'.");
        ...
        setUploadStatus({ type: 'success', message: `${newVoters.length} entries added.` });
      } catch (err: any) {
  ...
  const handleClearAll = async () => {
    if (confirm('Delete all live voting data permanently?')) {
      setIsSyncing(true);
      try { await onClearAll(); setUploadStatus({ type: 'success', message: "Data wiped." }); }
      catch (err) { setUploadStatus({ type: 'error', message: "Clear failed." }); }
  ...
  return (
    <div className="space-y-8 pb-12 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-black text-[#7b2b2a] uppercase tracking-tighter">Admin Console</h2>
          <p className="text-slate-500 font-medium">Real-time election tracking and registration.</p>
        </div>
      ...
      {activeTab === 'stats' && (
        <div className="grid md:grid-cols-2 gap-8">
          <div className="md:col-span-2 grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: 'Registry', val: voters.length, icon: Users, color: 'bg-[#7b2b2a]' },
              { label: 'Participated', val: voters.filter(v => v.used).length, icon: CheckCircle2, color: 'bg-[#c5a059]' },
              { label: 'Outstanding', val: voters.filter(v => !v.used).length, icon: Ticket, color: 'bg-slate-800' },
              { label: 'Turnout', val: `${voters.length ? Math.round((voters.filter(v => v.used).length / voters.length) * 100) : 0}%`, icon: BarChart3, color: 'bg-[#7b2b2a]' },
            ].map((stat, i) => (
      ...
      {activeTab === 'voters' && (
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="p-6 border-b border-[#faf7f2] flex flex-col sm:flex-row justify-between items-center gap-4">
            <h3 className="text-sm font-black text-[#7b2b2a] uppercase tracking-widest">Master Registry</h3>
            <div className="relative w-full sm:w-64">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
              <input type="text" placeholder="Filter Identity..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 rounded-xl border border-[#faf7f2] bg-[#faf7f2]/50 focus:border-[#c5a059] outline-none text-xs font-bold" />
            </div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-[#faf7f2]/50 text-slate-400 text-[9px] font-black uppercase tracking-[0.2em]">
                <tr><th className="px-6 py-4">Identity</th><th className="px-6 py-4">Wing</th><th className="px-6 py-4">Token</th><th className="px-6 py-4">Sync Status</th></tr>
              </thead>
              <tbody className="divide-y divide-[#faf7f2]">
                {filteredVoters.map((voter) => (
                  <tr key={voter.id} className="hover:bg-[#faf7f2]/30 transition-colors">
                    ...
                    <td className="px-6 py-4">{voter.used ? <span className="text-[#c5a059] font-black text-[9px] uppercase tracking-widest flex items-center gap-1"><CheckCircle2 size={12} /> Confirmed</span> : <span className="text-slate-300 font-black text-[9px] uppercase tracking-widest">Awaiting</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 space-y-8">
            ...
            <div className="p-10 border-4 border-dashed border-[#faf7f2] rounded-3xl bg-[#faf7f2]/40 flex flex-col items-center text-center">
              ...
              <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-6">Import CSV List (Name, Role)</p>
          ...
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 space-y-8">
            ...
            <div className="space-y-4">
              <button onClick={handleClearAll} className="w-full flex items-center justify-center gap-2 border-2 border-rose-100 text-rose-600 hover:bg-rose-50 px-4 py-4 rounded-3xl font-black text-[10px] uppercase tracking-widest transition-all"><Trash2 size={16} /> Wipe Cloud Database</button>
              <div className="bg-rose-50 p-4 rounded-2xl border border-rose-100 flex gap-3">
                ...
                <p className="text-[9px] text-rose-800 font-bold uppercase tracking-wider leading-relaxed">Danger: This wipes all distributed tokens. In-circulation physical passes will become invalid immediately.</p>
              </div>
            </div>
          </div>
        </div>
