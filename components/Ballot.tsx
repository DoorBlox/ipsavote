...
const Ballot: React.FC<BallotProps> = ({ voter, maleCandidates, femaleCandidates, onSubmit, onCancel }) => {
      ...
      <div className="flex flex-col items-center gap-8">
        <div className="relative shrink-0">
          <img 
            ...
            alt={candidate.name} 
            className={`w-64 h-64 sm:w-72 sm:h-72 rounded-[2rem] object-cover shadow-2xl transition-all duration-500 ${isSelected ? 'ring-8 ring-white' : 'grayscale-[15%] group-hover:grayscale-0'}`}
          />
          {isSelected && (
            <div className="absolute -top-4 -right-4 bg-[#7b2b2a] text-white p-4 rounded-full border-4 border-white shadow-2xl scale-110 z-10 animate-in zoom-in">
              <Check size={32} strokeWidth={4} />
            </div>
          )}
        </div>
        <div className="text-center min-w-0">
          <p className="text-[10px] text-[#c5a059] font-black uppercase tracking-[0.4em] mb-4">Official Candidate</p>
          <h4 className="text-3xl font-black text-slate-900 leading-tight mb-2 truncate">{candidate.name}</h4>
          <div className="flex items-center justify-center gap-3 py-1">
            <span className="text-[#c5a059] font-black text-sm uppercase tracking-widest">&</span>
            <h4 className="text-3xl font-black text-slate-900 leading-tight truncate">{candidate.viceName}</h4>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="max-w-6xl mx-auto animate-in slide-in-from-right-4 duration-500">
      <div className="bg-[#7b2b2a] rounded-t-[3rem] p-10 text-white border-b-8 border-[#c5a059]">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
          <div>
            <h2 className="text-4xl font-black mb-1 uppercase tracking-tight">Official Ballot</h2>
      ...
      <div className="bg-white rounded-b-[3rem] shadow-2xl p-10 sm:p-16 space-y-24">
        ...
        {(voter.role === UserRole.MALE || voter.role === UserRole.TEACHER) && (
          <div className="space-y-12">
            <div className="flex items-center justify-between border-b-4 border-[#faf7f2] pb-6">
              ...
              <div className="bg-[#7b2b2a]/5 text-[#7b2b2a] text-[10px] px-5 py-2 rounded-full font-black uppercase tracking-widest border border-[#7b2b2a]/10">Ticket</div>
            </div>
        ...
        {(voter.role === UserRole.FEMALE || voter.role === UserRole.TEACHER) && (
          <div className="space-y-12">
            <div className="flex items-center justify-between border-b-4 border-[#faf7f2] pb-6">
              ...
              <div className="bg-[#c5a059]/10 text-[#7b2b2a] text-[10px] px-5 py-2 rounded-full font-black uppercase tracking-widest border border-[#c5a059]/20">Ticket</div>
            </div>
        ...
        <div className="bg-[#faf7f2] p-8 rounded-[2rem] border-2 border-dashed border-[#c5a059]/40 flex items-start gap-6">
          ...
          <p className="text-xs text-[#7b2b2a] font-bold leading-relaxed tracking-wider uppercase">
            Finalize your choices. Submitting this ballot is permanent and uses your unique token.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6 pt-6">
          <button 
            ...
            className={`
              w-full sm:flex-1 py-7 px-12 rounded-3xl font-black text-2xl uppercase tracking-[0.2em] transition-all shadow-2xl
              ${canSubmit() 
          ...
          <button 
            onClick={onCancel}
            className="w-full sm:w-auto py-7 px-12 text-[#7b2b2a]/60 hover:text-[#7b2b2a] font-black uppercase tracking-widest transition-all"
          >
            Cancel Selection
          </button>
        </div>
